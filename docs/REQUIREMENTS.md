...
Agents run as native Node.js subprocesses with full access to host dev tools. Each group has its own isolated working directory and session. Security relies on per-group isolation and IPC authorization.
...
- **Native subprocesses** for agent execution
...
- Each agent gets its own working directory and HOME
- Full access to host dev tools (git, node, python, etc.)
- Browser automation via agent-browser
...
- Groups can have custom configuration via `containerConfig`
...
### Scheduler
- Built-in scheduler runs on the host, spawns agent subprocesses for task execution
- Custom `nanoclaw` MCP server provides scheduling tools
- Tools: `schedule_task`, `list_tasks`, `pause_task`, `resume_task`, `cancel_task`, `send_message`
- Tasks stored in SQLite with run history
- Scheduler loop checks for due tasks every minute
- Tasks execute Claude Agent SDK in the group's context
